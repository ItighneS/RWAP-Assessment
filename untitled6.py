...
Original file is located at
    https://colab.research.google.com/drive/1RdFAZd5TMcvIcDGRYY-6V7qEjU0ixxJe
"""
...
src = st.sidebar.radio("Choose data source", ["Upload CSV", "Local path"], index=1)  # Default: Local path
df = None
if src == "Upload CSV":
    ...
    # Default to your dataset path
    path = st.sidebar.text_input(
        "Local CSV path",
        value=r"C:\Users\adhik\Downloads\task1_valuations_with_preds.csv"
    )
    if path and os.path.exists(path):
        df = load_csv(path)
    else:
        st.sidebar.warning("Path not found. Upload a CSV or set a valid path.")
...
# Choose which valuation to visualize
val_options = []
...
if "Real Property Asset Type" in dff.columns:
    kpi_box("Asset Types (distinct)", dff["Real Property Asset Type"].nunique())
else:
    kpi_box("Asset Types (distinct)", None)
if has_geo:
    kpi_box("Geocoded Assets", int(dff["Latitude"].notna().sum()))
